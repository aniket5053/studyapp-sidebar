...
export function AddHomeworkForm({ classId }: { classId: string }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!homework.title || !homework.dueDate) {
      toast({
        ...
        description: "Please provide a title and due date for the homework",
        variant: "destructive",
      })
      return
    }

    // Get existing homework from localStorage
    const existingHomework = localStorage.getItem(`${classId}-homeworks`)
    const homeworks = existingHomework ? JSON.parse(existingHomework) : []
    ...
    // Add to localStorage
    localStorage.setItem(`${classId}-homeworks`, JSON.stringify([...homeworks, newHomework]))
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      setHomework({
        ...homework,
        files: [...homework.files, ...Array.from(e.target.files)],
      })
    }
  }
  ...
  const removeFile = (index: number) => {
    setHomework({
      ...homework,
      files: homework.files.filter((_, i) => i !== index),
    })
  }
    ...
    <div className="max-w-3xl mx-auto p-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold font-display dark:text-white">Add Homework</h1>
          <p className="text-slate-500 dark:text-slate-400">
            Create a new homework assignment for {classData.code}: {classData.name}
          </p>
        </div>

        <Card className="glass-morphism">
          <CardHeader>
            <CardTitle className="dark:text-white">Homework Details</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <Input
                  id="title"
                  placeholder="e.g., Homework #1: Linear Algebra"
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="description"
                  placeholder="Enter a description of the homework assignment..."
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setHomework({ ...homework, dueTime: e.target.value })}
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => setHomework({ ...homework, status: e.target.value })}
                >
